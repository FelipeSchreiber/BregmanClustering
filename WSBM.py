...
class BregmanBenchmark():
    def __init__(self,P,communities_sizes,min_,max_,dims=2,weight_variance=1,att_variance=1,\
                 weight_distribution="gamma",attributes_distribution="gaussian"):
        ...
        self.weight_distribution,self.get_params = distributions_dict[weight_distribution]
        self.att_distribution,self.get_params = distributions_dict[attributes_distribution]          
        self.dims = dims
    ...
    def generate_WSBM(self):
        ...
        params = self.get_params(means,self.weight_variance,self.n_clusters)
        ## get weights
        X = np.zeros((N,N))
        for i in range(N):
            for j in range(i,N):
    ...
    def generate_attributes(self):
        N = np.sum(self.communities_sizes)
        canonical_basis = np.eye(self.n_clusters)
        Y = np.zeros((N,self.n_clusters))
        params = self.get_params(canonical_basis[np.triu_indices(self.n_clusters)],self.weight_variance,self.n_clusters)
        cumsum = np.cumsum(self.communities_sizes)
        cumsum = np.insert(cumsum,0,0)
        for q,clus_len in enumerate(self.communities_sizes):
            for l in range(self.n_clusters):
                p = params[min(q,l)][max(q,l)] 
                Y[cumsum[q]:cumsum[q+1],l] = self.att_distribution(*p,size=clus_len)
        if self.dims > self.n_clusters:
            Y = np.hstack([Y,np.random.normal(size=(N,self.dims-self.n_clusters))])
        if self.dims < self.n_clusters:
            Y = TSNE(n_components=self.dims,).fit_transform(Y)
        return Y
