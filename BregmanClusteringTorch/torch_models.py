...
class BregmanEdgeClusteringTorch( BaseEstimator, ClusterMixin ):
    ...
    def fit( self, A, X, Y, Z_init=None ):
        ...
        if Z_init is None:
            ...
            self.predicted_memberships = torch.tensor(self.predicted_memberships).type(dtype)
        else:
        ...
        X = X.to_sparse()
        self.edge_index = torch.nonzero(A)
    ...
    def assignments( self, A, X, Y ):
        z = torch.zeros( (Y.shape[ 0 ],self.n_clusters)).type(dtype)
